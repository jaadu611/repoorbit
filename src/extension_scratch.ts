...
async function discoverLS() {
  try {
    ...
    let ports: string[] = [];
    try {
      const ss = execSync(`ss -tunlp 2>/dev/null | grep "pid=${pid}"`, {
        encoding: "utf8",
...
async function secureRPCRequest(
  url: string,
  options: any,
): Promise<{
  ok: boolean;
...
async function fetchModelsHybrid(): Promise<
  Array<{ id: string; name: string; vendor?: string; quota?: string | number }>
> {
    ...
    for (const port of ls.ports) {
      try {
        ...
        for (const proto of ["https", "http"]) {
          try {
            const res = await secureRPCRequest(
              `${proto}://127.0.0.1:${port}${endpoint}`,
              {
                method: "POST",
                headers: {
                ...
                modelCache = cfgs
                  .map((c: any) => {
                    ...
                    return {
                      ...
                      name: lbl,
                      vendor: lbl.toLowerCase().includes("gemini")
                        ? "Google"
                        : lbl.toLowerCase().includes("claude")
                          ? "Anthropic"
                          : lbl.toLowerCase().includes("gpt")
                            ? "OpenAI"
                            : "Unknown",
                      quota,
                      resetTime,
                  ...
                  .filter(
                    (m: any) => !m.name.toLowerCase().includes("internal"),
                  );
                cacheExpiry = Date.now() + CACHE_TTL;
                console.log(
                  `[RepoOrbit] Models fetched via Antigravity LS RPC (${proto})`,
                );
                return modelCache || [];
              }
            }
  ...
  modelCache = [
    ...
    {
      id: "MODEL_PLACEHOLDER_M35",
      name: "Claude Sonnet 4.6",
      vendor: "Anthropic",
    },
    {
      id: "MODEL_OPENAI_GPT_OSS_120B_MEDIUM",
      name: "GPT-OSS 120B",
      vendor: "OpenAI",
    },
  ];
...
async function sendAntigravityChatDirect(
  ...
  modelId: string,
  repoContext?: any,
  sessionState?: { cascadeId: string | null; protocol: string },
  onUpdate?: (data: { text: string; steps?: any[] }) => void,
): Promise<{ text: string; cascadeId: string; protocol: string }> {
  const ls = await discoverLS();
  ...
  let cascadeId = sessionState?.cascadeId || null;
  let protocol = sessionState?.protocol || "https";
  ...
  if (!cascadeId) {
    ...
    try {
      startRes = await secureRPCRequest(
      ...
      if (!startRes.ok && startRes.status === 400) {
        ...
        if (text.includes("HTTP request to an HTTPS server"))
          protocol = "https"; // Already used https
        else if (text.includes("HTTPS request to an HTTP server"))
          protocol = "http";
      }
    } catch (err) {
      protocol = "http";
    ...
    cascadeId = resData.cascadeId;
    if (!cascadeId) throw new Error("No cascadeId returned");

    // Track and open the newly created background conversation in the IDE UI
    try {
      await vscode.commands.executeCommand("antigravity.trackBackgroundConversationCreated", cascadeId);
      await vscode.commands.executeCommand("antigravity.openAgent", cascadeId);
      console.log(`[RepoOrbit] Tracked and opened newly created background conversation in IDE: ${cascadeId}`);
    } catch (trackErr) {
      console.warn("[RepoOrbit] Failed to track/open newly created background conversation:", trackErr);
    }
  }

  // 2. Send Message
  const sendRes = await secureRPCRequest(
    `${protocol}://127.0.0.1:${port}/exa.language_server_pb.LanguageServerService/SendUserCascadeMessage`,
    {
      method: "POST",
      headers: {
        "Content-Type": "application/json",
  ...
  if (!sendRes.ok) {
    ...
    console.error(
      `[RepoOrbit] SendMessage failed with status ${sendRes.status}:`,
      errText,
    );
  ...
  while (true) {
    ...
    try {
      const trajRes = await secureRPCRequest(
        `${protocol}://127.0.0.1:${port}/exa.language_server_pb.LanguageServerService/GetCascadeTrajectory`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
      ...
      if (trajRes.ok) {
        ...
        for (const s of steps) {
          ...
          if (
            ...
            try {
              await secureRPCRequest(
                `${protocol}://127.0.0.1:${port}/exa.language_server_pb.LanguageServerService/RespondToToolCall`,
                {
                  method: "POST",
                  headers: {
                    "Content-Type": "application/json",
        ...
        const plannerStep = [...steps]
          .reverse()
          .find((s: any) => s.type === "CORTEX_STEP_TYPE_PLANNER_RESPONSE");
        if (plannerStep) {
        ...
        if (
          (status === "CASCADE_RUN_STATUS_IDLE" || status === 2) &&
          lastText
        ) {
          return { text: lastText, cascadeId, protocol };
        }

        // If an error step appeared, fail fast and DO NOT continue polling
        if (
          steps.some((s: any) => s.type === "CORTEX_STEP_TYPE_ERROR_MESSAGE")
        ) {
          const errStep = steps.find(
            (s: any) => s.type === "CORTEX_STEP_TYPE_ERROR_MESSAGE",
          );
          const errorMsg =
            errStep?.errorMessage?.error?.userErrorMessage ||
      ...
      console.warn(
        `[RepoOrbit] Poll attempt ${pollCount} result:`,
        pollErr.message,
      );
      // If it's a quota error, throw it immediately to break the loop
      if (
        pollErr.message.includes("exhausted") ||
        pollErr.message.includes("quota") ||
        pollErr.message.includes("capacity")
      ) {
        throw pollErr;
      }
...
export async function activate(context: vscode.ExtensionContext) {
  ...
  // DEBUG: List all VS Code commands to commands_debug.json
  Promise.resolve(vscode.commands.getCommands(true)).then(allCmds => {
    const filtered = allCmds.filter(c => c.toLowerCase().includes('antigravity') || c.toLowerCase().includes('chat') || c.toLowerCase().includes('codeium'));
    fs.writeFileSync(
      path.join(context.extensionPath, 'commands_debug.json'),
      JSON.stringify(filtered, null, 2)
    );
    console.log('[RepoOrbit] Successfully dumped filtered commands to commands_debug.json');
  }).catch(err => {
    console.error('[RepoOrbit] Failed to dump commands:', err);
  });

  // ─── Suppress IDE Errors ───
  const isFileGitIgnoredCmd = vscode.commands.registerCommand(
  ...
  let activeProtocol: string = "https";

  const updateTokenCmd = vscode.commands.registerCommand(
    "repoorbit.updateToken",
    async () => {
        ...
        switch (message.command) {
          ...
          case "chat":
            try {
              ...
              console.log(
                `[RepoOrbit] AI Chat Request [Model: ${modelId}]:`,
                query,
              );

              // Force open the chat panel in the UI
              currentPanel.webview.postMessage({ command: "forceOpenChat" });

              // Force open both the view container and the chat view unconditionally in the IDE sidebar
              try {
                const cmds = await vscode.commands.getCommands(true);
                // 1. Find view container commands
                const viewContainerCmds = cmds.filter(c => 
                  (c.toLowerCase().includes("antigravity") || c.toLowerCase().includes("codeium")) && 
                  c.startsWith("workbench.view.extension.")
                );
                for (const vcCmd of viewContainerCmds) {
                  console.log("[RepoOrbit] Focusing view container via command:", vcCmd);
                  await vscode.commands.executeCommand(vcCmd);
                }

                // 2. Execute explicit non-toggle open/focus panel commands
                const explicitCmds = [
                  "antigravity.agentSidePanel.open",
                  "antigravity.agentSidePanel.focus",
                  "antigravity.openAgent"
                ];
                for (const expCmd of explicitCmds) {
                  if (cmds.includes(expCmd)) {
                    console.log("[RepoOrbit] Executing explicit focus command:", expCmd);
                    await vscode.commands.executeCommand(expCmd);
                  }
                }

                // 3. Find other chat focus commands (excluding toggles)
                const chatFocusCmds = cmds.filter(c => 
                  (c.toLowerCase().includes("antigravity") || c.toLowerCase().includes("codeium")) && 
                  c.includes("chat") && 
                  (c.includes("focus") || c.includes("open") || c.includes("show")) &&
                  !c.toLowerCase().includes("toggle")
                );
                for (const cfCmd of chatFocusCmds) {
                  console.log("[RepoOrbit] Focusing chat view via command:", cfCmd);
                  await vscode.commands.executeCommand(cfCmd);
                }

                // 4. Fallback generic focus (excluding toggle commands)
                const chatCmd = cmds.find(c => 
                  c.includes("antigravity") && 
                  c.includes("chat") && 
                  !c.toLowerCase().includes("toggle")
                ) || cmds.find(c => 
                  c.includes("chat.focus") && 
                  !c.toLowerCase().includes("toggle")
                );
                if (chatCmd) {
                  console.log("[RepoOrbit] Fallback focusing chat view:", chatCmd);
                  await vscode.commands.executeCommand(chatCmd);
                }

                // 5. Open specific active conversation if already created
                if (activeCascadeId) {
                  try {
                    await vscode.commands.executeCommand("antigravity.trackBackgroundConversationCreated", activeCascadeId);
                    await vscode.commands.executeCommand("antigravity.openAgent", activeCascadeId);
                    console.log(`[RepoOrbit] Tracked and opened existing conversation: ${activeCascadeId}`);
                  } catch (openErr) {
                    console.warn("[RepoOrbit] Failed to track/open existing conversation:", openErr);
                  }
                }
              } catch (cmdErr) {
                console.warn("[RepoOrbit] Failed to focus native chat panel:", cmdErr);
              }

              // 1. Try Direct Cascade Flow (Best UX, response stays in RepoOrbit)
              try {
                const result = await sendAntigravityChatDirect(
                  query,
                  modelId,
                  repoContext,
                  { cascadeId: activeCascadeId, protocol: activeProtocol },
                  (update) => {
                    currentPanel?.webview.postMessage({
                      command: "chatStream",
                      text: update.text,
                ...
                activeCascadeId = result.cascadeId;
                ...
                console.warn(
                  "[RepoOrbit] Direct Cascade Flow failed:",
                  lsErr.message,
                );

                // If it's a quota/exhaustion error, DO NOT fall back. Just show it.
                if (
                  lsErr.message.includes("exhausted") ||
                  lsErr.message.includes("quota") ||
                  lsErr.message.includes("capacity")
                ) {
                  currentPanel.webview.postMessage({
                    command: "chatError",
                    text: lsErr.message,
                ...
                console.log(
                  "[RepoOrbit] Trying fallbacks for non-quota error...",
                );
              }
              ...
              if (vscode.lm) {
                try {
                  const models = await (vscode as any).lm.selectChatModels({
                    family: modelId.includes("gemini") ? "gemini" : undefined,
                  });
                  const model =
                    models.find((m: any) => m.id === modelId) || models[0];
                  if (model) {
                    const request = [
                      new (vscode as any).LanguageModelUserMessage(query),
                    ];
              ...
              const chatCmd =
                cmds.find(
                  (c) => c.includes("antigravity") && c.includes("chat"),
                ) || cmds.find((c) => c.includes("chat.focus"));

              if (chatCmd) {
                console.log(
                  "[RepoOrbit] Forwarding to discovered command:",
                  chatCmd,
                );
                ...
                throw new Error(
                  "All chat providers failed and no native chat command found.",
                );
              }
            } catch (err: any) {
              currentPanel.webview.postMessage({
          ...
          case "analyzeRepo":
            try {
              ...
              const storedToken =
                context.globalState.get<string>("github_token");
          ...
          case "cloneRepo":
            try {
              ...
              try {
                ...
                console.error(
                  "[RepoOrbit] Failed to bootstrap rules/queries:",
                  bootstrapErr,
                );
              }
              // ---------------------------------
              ...
              vscode.window.showInformationMessage(
                `RepoOrbit: Cloned ${url} successfully!`,
              );
              currentPanel.webview.postMessage({
                command: "cloneSuccess",
                path: cloneDest,
              });
            } catch (err: any) {
          ...
          case "checkWorkspaceStatus":
            try {
              ...
              const meaningfulFiles = files.filter(
                (f) =>
                  ![".git", ".DS_Store", ".vscode", ".antigravity"].includes(f),
              );
              currentPanel.webview.postMessage({
                command: "workspaceStatus",
  ...
  context.subscriptions.push(updateTokenCmd, openWorkspaceCmd);
}
...
<body style="margin:0;padding:0;background:#0a0a0a;color:white;">
  ...
  if (!fs.existsSync(indexHtmlPath))
    return "<h1>Build missing. Run `npm run ext:build-ui`</h1>";
  ...
  html = html
    .replace(/(href|src)="\/assets\//g, `$1="${assetUri}/assets/`)
    .replace(/(href|src)="\//g, `$1="${assetUri}/`);
  return html;
}
